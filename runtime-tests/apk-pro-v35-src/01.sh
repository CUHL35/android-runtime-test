...
cat > "$ROOT/app/build.gradle" <<'APKPRO_baeccf9ebd869352'
plugins {
...
APKPRO_baeccf9ebd869352
mkdir -p "$(dirname "$ROOT/app/src/main/AndroidManifest.xml")"
cat > "$ROOT/app/src/main/AndroidManifest.xml" <<'APKPRO_98ff4d572bc759ec'
...
APKPRO_98ff4d572bc759ec
mkdir -p "$(dirname "$ROOT/app/src/main/assets/engine/SigningKeyCheck.java")"
cat > "$ROOT/app/src/main/assets/engine/SigningKeyCheck.java" <<'APKPRO_9d2e6d5863efda0a'
import java.io.File;
...
APKPRO_9d2e6d5863efda0a
mkdir -p "$(dirname "$ROOT/app/src/main/assets/engine/provision-packages.sh")"
cat > "$ROOT/app/src/main/assets/engine/provision-packages.sh" <<'APKPRO_27725707e6d0db9c'
#!/system/bin/sh
set -eu
